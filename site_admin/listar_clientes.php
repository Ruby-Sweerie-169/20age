...
$stmt = $conn->prepare("SELECT id, nome, email, tlm, username, password FROM users ORDER BY nome");
$stmt->execute();
...
<body class="bg-[#D1DEC6] min-h-screen flex flex-col">
    ...
    <main class="flex-grow p-8">
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-2 text-center"><?php echo $row['nome']; ?></td>
                                <td class="px-4 py-2 text-center"><?php echo $row['email']; ?></td>
                                <td class="px-4 py-2 text-center"><?php echo $row['tlm']; ?></td>
                                <td class="px-4 py-2 text-center"><?php echo $row['username']; ?></td>
                                <td class="px-4 py-2 text-center">
                                    <a href="editar_cliente.php?id=<?php echo $row['id']; ?>" class="bg-[#B67272] text-white font-bold px-4 py-2 rounded-lg hover:bg-[#754949] transition">Editar</a>
                                    <span class="mx-2">|</span>
                                    <a href="eliminar_cliente.php?id=<?php echo $row['id']; ?>" class="bg-red-600 text-white font-bold px-4 py-2 rounded-lg hover:bg-red-700 transition">Eliminar</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
            ...
            <?php else: ?>
                <p class="text-center text-[#B67272]">Nenhum usuário encontrado.</p>
            <?php endif; ?>
        </div>
